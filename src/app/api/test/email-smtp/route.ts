...
import { generateOTP, sendOTPEmailSMTP, EmailTemplate } from '@/lib/email-smtp';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!result.success) {
      return NextResponse.json(
        { error: `Failed to send ${testTemplate} email via SMTP: ${result.error}` },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: `${testTemplate} email sent successfully via SMTP`,
      template: testTemplate,
      purpose: testPurpose,
...
// Send all three templates at once via SMTP
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'All test emails sent via SMTP',
      results
    });

  } catch (error) {
    console.error('Test all SMTP emails error:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
