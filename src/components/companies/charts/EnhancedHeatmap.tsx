...
export default function EnhancedHeatmap({
  ...
  const {
    ...
    companyId
      ? `/api/charts/travel-data?companyId=${companyId}`
  ...
  useEffect(() => {
    ...
    function getISOWeek(date) {
      ...
      if (target.getDay() !== 4) {
        // If January 1st is not a Thursday
    ...
    console.log(
      `📅 HEATMAP: Current date: ${now.toDateString()}, ISO Week: ${weekNumber}`,
    );
  ...
  useEffect(() => {
    ...
    Object.entries(rawData.yearlyData).forEach(
      ([yearStr, yearData]: [string, any]) => {
        const year = parseInt(yearStr);
        const entities = new Map<string, EntityData>();

        Object.entries(yearData.entities).forEach(
          ([entityName, entityData]: [string, any]) => {
            const weeks = new Map<number, WeekData>();

            Object.entries(entityData.weeks).forEach(
              ([weekStr, weekData]: [string, any]) => {
  ...
  const renderWeekNumbers = () => {
    ...
    for (let week = 1; week <= 52; week++) {
      ...
      const isRegularInterval =
        week === 1 || (week >= 5 && (week - 5) % 5 === 0);
      const isTooCloseToCurrentWeek =
        Math.abs(week - currentWeek) <= 2 && week !== currentWeek;

      // Show regular intervals unless they're too close to current week, or always show current week
      const showWeekNumber =
        (isRegularInterval && !isTooCloseToCurrentWeek) || isCurrentWeek;
  ...
  const getFixedSizing = (containerRef?: React.RefObject<HTMLDivElement>) => {
    ...
    if (typeof window !== "undefined" && containerRef?.current) {
      ...
      return {
        ...
        squareSize: weekly.squareSize,
        gap: weekly.gap,
        nameColumnWidth,
        verticalGap: weekly.verticalGap,
      };
    }
  ...
  const [fixedSizing, setFixedSizing] = useState({
    mode: "weekly" as const,
    squareSize: 10,
    gap: 1,
    nameColumnWidth: 150,
  ...
  if (isLoading) {
    return (
      <div
        className={`bg-white dark:bg-[#1a1a1a] rounded-lg border border-pearl-200 dark:border-[#2a2a2a] p-6 ${className}`}
      >
  ...
  if (
    !heatmapData ||
    (heatmapData.yearlyData.size === 0 && selectedSection !== "wolthers")
  ) {
    return (
      <div
        className={`bg-white dark:bg-[#1a1a1a] rounded-lg border border-pearl-200 dark:border-[#2a2a2a] p-6 ${className}`}
  ...
  console.log(
    `📈 HEATMAP RENDER: currentWeek=${currentWeek}, selectedSection=${selectedSection}`,
  );
      ...
      <div className="w-full overflow-x-auto" style={{ maxWidth: "100%" }}>
        <div
          style={{
            ...
            gap: `${fixedSizing.verticalGap}px`,
          }}
        >
            ...
            .map((year, index, array) => {
              ...
              Array.from(yearData.entities.values()).forEach((entity) => {
                entity.weeks.forEach((weekData, week) => {
                  yearWeeklyData.set(
                    week,
                    (yearWeeklyData.get(week) || 0) + weekData.count,
                  );
                });
              });
              ...
              const yearEntityData: EntityData = {
                name: year.toString(),
                weeks: new Map(
                  Array.from(yearWeeklyData.entries()).map(([week, count]) => [
                    week,
                ...
                <div key={year} className="space-y-px">
                  ...
                  {isExpanded && (
                    <div className="space-y-px">
                      {Array.from(yearData.entities.entries()).map(
                        ([entityName, entityData]) => (
                          <div
                            key={entityName}
                            className="flex items-center gap-2"
                            style={{ height: "10px", paddingLeft: "32px" }}
                          >
                            <div
                              className="text-xs text-gray-600 dark:text-gray-400 flex items-center"
                              style={{
                                width: `${fixedSizing.nameColumnWidth - 32}px`,
                                flexShrink: 0,
                              }}
                            >
          ...
          {fixedSizing.mode === "weekly" && (
            <div className="flex items-center gap-2 mt-1">
              ...
              <div className="flex overflow-hidden">{renderWeekNumbers()}</div>
            </div>
          )}
