    ...
    try {
      ...
      if (response.ok) {
        ...
        if (dynamicRegions && dynamicRegions.length > 0) {
          // Merge base regions with dynamically discovered regions, avoiding duplicates
          const combinedRegions = [...BASE_COFFEE_REGIONS]
          
          dynamicRegions.forEach((dynRegion: any) => {
            // Check for duplicates by both ID and name to handle different ID formats
            const existingRegion = combinedRegions.find(r => 
              r.id === dynRegion.id || 
              (r.name === dynRegion.name && r.country === dynRegion.country)
            )
            ...
            if (!existingRegion) {
              ...
              // Update company count from database if higher
              existingRegion.estimatedCompanies = Math.max(
                existingRegion.estimatedCompanies, 
                dynRegion.estimatedCompanies || 0
              )
            }
          })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center">
        <h3 className="text-lg font-medium mb-2 text-gray-900 dark:text-golden-400">
          AI-Powered Regional Company Discovery
        </h3>
        <p className="text-sm text-gray-700 dark:text-gray-300">
          Select a coffee region to automatically discover and suggest companies
        </p>
      </div>
          ...
          return (
            <div
              ...
              className={`
                ...
                ${isSelected 
                  ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-800/20 dark:border-emerald-400' 
                  : 'border-gray-200 hover:border-gray-300 bg-white dark:bg-[#1a1a1a] dark:border-[#2a2a2a] dark:hover:border-[#3a3a3a]'
                }
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center space-x-2">
                  ...
                  {region.country !== 'Brazil' && (
                    <span className="text-xs px-2 py-1 bg-gray-100 dark:bg-[#2a2a2a] text-gray-600 dark:text-gray-400 rounded-full">
                      {region.country}
                    </span>
                  )}
                </div>
              ...
              <div className="mb-3">
                ...
                <div className="flex flex-wrap gap-1">
                  {region.mainCities.slice(0, 3).map((city, index) => (
                    <span
                      key={index}
                      className="px-2 py-1 text-xs rounded-full bg-gray-100 dark:bg-[#2a2a2a] text-gray-600 dark:text-gray-300"
                    >
                      {city}
                    </span>
                  ))}
                  {region.mainCities.length > 3 && (
                    <span className="px-2 py-1 text-xs rounded-full bg-gray-100 dark:bg-[#2a2a2a] text-gray-500 dark:text-gray-400">
      ...
      {/* Custom AI Search */}
      <div className="border-t dark:border-[#2a2a2a] pt-6">
        <h4 className="font-medium mb-3 text-gray-900 dark:text-golden-400">
          Custom AI Search
        </h4>
        ...
        <div className="flex space-x-2">
          <input
            ...
            className="flex-1 px-3 py-2 border border-gray-300 dark:border-[#2a2a2a] rounded-lg bg-white dark:bg-[#1a1a1a] text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          />
          <button
